...
<template>
  <div class="flex">
    ...
    <!--<InputFile/>-->
    <Button
      class="p-button-link w-1/10"
      @click="loadExample"
      label="Load example"
    />
  </div>

  <Loader class="p-mt-2" v-if="loaded && uniprotDBFilled && !xlsDropped && proteomes.length == 0" message="Scanning proteomes..." />
  <div v-if="loaded && uniprotDBFilled && !xlsDropped && proteomes.length !== 0">
    Select proteome

    <Dropdown 
      v-model="selectedProteome"
      :options="proteomes"
      optionLabel="name"
  ...
  <Button
    v-if="selectedProteome.name"
    label="Load data into table"
  ...
  <Loader
    class="p-mt-2"
    v-if="loaded && !uniprotDBFilled"
    message="Uniprot data are stored..."
  />
  <div
    v-if="loaded && uniprotDBFilled && !xlsDropped && canShowTable"
    class="mt-5"
  >
  ...
  setup(_, { emit }) {
    ...
    const storeInUniprotDatabase = (): Promise<boolean> => {
      ...
      return new Promise((res, rej) => {
        const uniprotIdList: string[] | undefined =
          store.getters.getColDataByName("Accession", "string");
        if (uniprotIdList) {
    ...
    const storeData = async (data: any) => {
      ...
      const col: ColTemplate[] = headers.map(
      ...
      const selectedCols = col.filter(
        (colElmt: any) =>
          (colElmt.field.includes("Abundance Ratio") &&
            !colElmt.field.includes("Variability")) ||
          colElmt.field === "Accession"
      );
